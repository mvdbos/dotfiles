...
alias gap='g add -p'
alias gb='g bv'
alias gbm='git for-each-ref --color=always --sort=-committerdate --format="%(committerdate:iso8601)----%(HEAD) %(color:yellow)%(align:width=40,position=left)%(refname:short)%(end) %(color:reset)%(subject) %(color:green)(%(committerdate:relative))%(color:reset)----%(authorname)" refs/heads/ | grep -v "backup" | grep "$(git config user.name)" | awk -F"----" '\''{print $2}'\'''
alias gc='g ci'
alias gco='g co'
