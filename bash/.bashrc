...
alias gbm='git for-each-ref --color=always --sort=-committerdate --format="%(committerdate:iso8601)----%(HEAD) %(color:yellow)%(align:width=40,position=left)%(refname:short)%(end) %(color:reset)%(subject) %(color:green)(%(committerdate:relative))%(color:reset)----%(authorname)" refs/heads/ | grep -v "backup" | grep "$(git config user.name)" | grep -v year | grep -v month | awk -F"----" '\''{print $2}'\'''
alias gb='g br'
alias gc='g ci'
alias gco='g co'
